// Portions adapted from blazen-audio-vc 0.7.0 (MPL-2.0).
// Source: https://crates.io/crates/blazen-audio-vc/0.7.0
...
impl HubertBase {
    ...
    pub(super) fn forward_layers(
        ...
        let inner = || -> candle_core::Result<Vec<Tensor>> {
            ...
            let h = self.feature_extractor.forward(&xs).map_err(|e| {
                candle_core::Error::Msg(format!("ContentVec feature extractor: {e}"))
            })?;
            ...
            let h = self.feature_projection.forward(&h).map_err(|e| {
                candle_core::Error::Msg(format!("ContentVec feature projection: {e}"))
            })?;
            ...
            //    `super::hubert_encoder::PosConv`.
            let pos = self.pos_conv.forward(&h).map_err(|e| {
                candle_core::Error::Msg(format!("ContentVec position convolution: {e}"))
            })?;
            let h = (&h + &pos)?;
            ...
            for (index, layer) in self.layers.iter().enumerate() {
                h = layer.forward(&h).map_err(|e| {
                    candle_core::Error::Msg(format!("ContentVec transformer layer {index}: {e}"))
                })?;
